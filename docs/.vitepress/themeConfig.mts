 ...
 * 动态生成文档目录
 */
const docsSidebar = docsDirList
  .map((name) => generateCatalog(path.resolve("./src/Docs"), `${name}`))
  .filter(Boolean);

/**
 * /src/Components/目录下的文件/文件夹
 * ps: 过滤掉非.md的文件 / 只是Docs目录下第一层
 */
const componentDirList = fse
.readdirSync("./src/Components")
.filter(
  (name) =>
    /** 过滤掉_前缀的目录 */
    (!name.startsWith("_") &&
      (fse.statSync(path.resolve("./src/Components", name)).isDirectory()) ||
      name.endsWith(".md"))
);
 ...
 * 动态生成组件目录
 */
const componentSidebar = componentDirList
  .map((name) => generateCatalog(path.resolve("./src/Components"), `${name}`, '/Components'))
  .filter(Boolean);
...
const themeConfig: UserConfig<DefaultTheme.Config>["themeConfig"] = {
  ...
  nav: [
    { text: "首页", link: "/" },
    { text: "文档", activeMatch: `^/Docs/`, link: getFirstRoute(docsSidebar) },
    { text: "组件", activeMatch: `^/Components/`, link: getFirstRoute(componentSidebar) },
    {
      text: "🔗github",
